...
CREATE TABLE tasksDependency(
	...
	CONSTRAINT pk_tasksDependency PRIMARY KEY(firstId, secondId),
	CONSTRAINT fk_td1_tasks FOREIGN KEY(firstId) REFERENCES tasks(id) ON DELETE CASCADE ON UPDATE CASCADE,
	CONSTRAINT fk_td2_tasks FOREIGN KEY(secondId) REFERENCES tasks(id) ON DELETE CASCADE ON UPDATE CASCADE
);
...
BEGIN
  cycled = 0;
  for select  td.FIRSTID from TASKSDEPENDENCY td where td.SECONDID = :master
      into master_id
  do
  begin
    if (master_id = slave) then cycled = 1;
    else execute procedure check_task_dependency (master_id, slave) returning_values cycled;
  end
END^
...
BEGIN 
    execute procedure CHECK_TASK_DEPENDENCY (new.FIRSTID, new.SECONDID) returning_values cycled; 
